...
func (app *Config) PostLoginPage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.Session.Put(r.Context(), "error", "Invalid credentials. (email)")
		log.Println(err)
	...
	if err != nil {
		app.Session.Put(r.Context(), "error", "Invalid credentials. (password matches)")
		http.Redirect(w, r, "/login", http.StatusSeeOther)
		return
	}
	...
	if !validPassword {
		...
		app.sendEmail(msg)
		app.Session.Put(r.Context(), "error", "Invalid credentials. (valid password)")
		http.Redirect(w, r, "/login", http.StatusSeeOther)
		return
	}
...
func (app *Config) PostRegisterPage(w http.ResponseWriter, r *http.Request) {
	...
	url := fmt.Sprintf("http://localhost:81/activate?email=%s", u.Email)
...
func (app *Config) ActivateAccount(w http.ResponseWriter, r *http.Request) {
	...
	testURL := fmt.Sprintf("http://localhost:81%s", url)
